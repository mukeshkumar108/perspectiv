...
import { findLessonByRouteId, sortLessons } from '@/src/lessons/model';
...
export default function LessonDetailScreen() {
  ...
  const sortedLessons = useMemo(
    () => sortLessons(lessonsQuery.data?.items ?? []),
    [lessonsQuery.data?.items],
  );
  const lesson = useMemo(
    () => findLessonByRouteId(sortedLessons, routeId),
    [routeId, sortedLessons],
  );

  const [isPlaying, setIsPlaying] = useState(false);
  const [positionMs, setPositionMs] = useState(0);
  const [durationMs, setDurationMs] = useState(
    (lesson?.durationSec ?? 60) * 1000,
  );
      ...
      try {
        ...
        const { sound } = await AudioModule.Sound.createAsync(
          ...
          (status: AVPlaybackStatus) => {
            ...
            setDurationMs(status.durationMillis ?? (lesson.durationSec ?? 60) * 1000);
        ...
        // Ignore load failure and keep controls disabled.
      }
    };
  ...
  }, [lesson?.audioUrl, lesson?.durationSec]);
  ...
  if (lessonsQuery.isLoading) {
    return (
      <ScreenContainer style={styles.container} ambient={false}>
        <Text variant="title">Loading lesson...</Text>
      </ScreenContainer>
    );
  }
    ...
    return (
      <ScreenContainer style={styles.container} ambient={false}>
        ...
        <View style={styles.missingActions}>
          <Button title="Back to lessons" variant="secondary" onPress={() => router.replace('/(main)/lessons' as any)} />
        </View>
      </ScreenContainer>
    );
  }
    ...
    >
      ...
      <Animated.View entering={motion.itemEnter(140, 'up')} style={styles.meta}>
        ...
        <View style={styles.metaRow}>
          ...
          <Text variant="small" color={theme.textSecondary}>
            {Math.max(1, Math.round(durationMs / 60_000))} min • streaming
          </Text>
        </View>
      </Animated.View>
      ...
      <Animated.View entering={motion.itemEnter(280, 'up')} style={styles.footer}>
        <Button title="Back to lessons" variant="secondary" onPress={() => router.back()} />
      </Animated.View>
    </ScreenContainer>
  );
}
